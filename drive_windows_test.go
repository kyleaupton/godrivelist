...
func TestWindowsDriveList(t *testing.T) {
        ...
        for _, drive := range drives {
                if strings.HasPrefix(drive.DisplayName, "C:") {
                        ...
                        // Test that it's a fixed drive
                        if !strings.Contains(drive.Description, "Local Disk") && !strings.Contains(drive.Description, "Windows") {
                                t.Error("C: drive not recognized as a local/system disk")
                        }
                        break
                }
        }
